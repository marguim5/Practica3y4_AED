...
```{r}
library(pacman)
packages=c("tidyverse", "knitr", "ggplot2", "datasets", "RColorBrewer",'dplyr','lubridate',"nycflights13","stringr")
...
```{r}
pew_religion<-gather(pew_religion, income, cases, -religion, factor_key=T) 
pew_religion %>%  head
...
```{r}
#unimos
billboard<-pivot_longer(raw,cols = 6:81,names_to = "week", values_to = "value", names_prefix = "wk")

#pasamos a variable fecha con lubridate
billboard$date.entered <-billboard$date.entered %>% as.Date(format = "%Y-%m-%d")

#limpiamos los NA
billboard<-billboard %>% drop_na

#creamos la columna date
semanas<-billboard$week %>% 
  as.numeric %>% weeks
billboard$date <- billboard$date.entered + semanas
...
```{r}
# Primero agrupamos todas las unidades
# Agrupamos todas las columnas
tb <- gather(raw, sexo, casos, 3:18)
head(tb)
# Luego separamos la variable Sexo en dos, Sexo y Edad
tb <- separate(tb, sexo, into=c("sexo", "edad"), sep = 1)
head(tb)
```
...
```{r}
#crearemos la variable día
weather<-raw %>% 
  pivot_longer(cols = 5:35, names_to = "dia", 
               values_to = "temp", names_prefix = "d", values_drop_na = T) %>%
  
  #ahora creamos las columnas tmax y tmin
  pivot_wider(names_from = element, values_from = temp) %>%
  
  #ahora creamos la columna fecha
  unite(col = "fecha",year, month, dia, sep = "-", remove = T)

weather$fecha <- weather$fecha %>% as_date(format = "%Y-%m-%d")
...
```{r }
planes.f <- planes %>%
  # primero, pasamos a minúsculas todas las letras para poder buscar mejor
  mutate(manufacturer = str_to_lower(manufacturer)) %>%
  # usamos una expresión regular para buscar por el único patrón que hay: douglas
  filter(str_detect(manufacturer, "^*duoglas*",negate=T)) %>%
  # ahora seleccionamos por número de asientos
  filter(between(seats,90,210)) %>%
  # finalmente unificamos las denominaciones airbus bajo airbus corps
  mutate(manufacturer = ifelse(str_detect(manufacturer, "airbus*"), 
                               "airbus comp", manufacturer))
...
```{r }
flights.f <- flights %>% semi_join(planes.f, by = "tailnum")
...
```{r }
flights <- flights %>% 
  mutate(Full_Date = paste(as.character(day),as.character(month),as.character(year),
                           sep = "-"),
                              .keep="unused" )
...
Distances_plane <- flights %>%
  ...
  #eligo trabajar con cada avion
  group_by(tailnum) %>%
  #para cada avion (grupo) le aplico estas funciones estadísticas
  summarise(
    Num_flights = n(),
